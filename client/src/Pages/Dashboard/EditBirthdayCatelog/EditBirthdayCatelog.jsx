...
import { updateBirthdayCakeDesaign } from "../../../Api/BirthdayRelatedApi/birthday";
...
const EditBirthdayCatelog = () => {
        ...
        const handleSubmit = async(e) => {
            ...
            if (cakeImage) {
              try {
                ...
                uploadedImageURL = imgbb?.data?.display_url;
              } catch (error) {
            ...
            const designData = {image:uploadedImageURL,name,price,availability,description}
            ...
            setLoading(true)
            try{
              const result = await updateBirthdayCakeDesaign(payload);
              console.log(result);
              if(result?.modifiedCount > 0){
                form.reset();
                toast.success('ধন্যবাদ,কেকের ফিল্ড আপডেট করা হয়েছে')
              }
            }
            catch(err){
              toast.error('দুঃখিত কেকের ফিল্ড আপডেট করা যায় নি। দয়া করে আবার চেষ্টা করুন')
            }
            finally{
              setLoading(false)
            }
    ...
    <form onSubmit={handleSubmit}>
        <div className="mb-5">
            ...
            <div className="">
            <h3 className="text-center uppercase text-xl mb-3 text-primary font-medium">* এখান থেকে বার্থডে কেকের ডিজাইন আপডেট করুন *</h3>
            ...
            <input name="image" className="border-slate-300 border focus:outline-none bg-transparent px-4 py-2 rounded-lg text-[16px] w-full" type="file"/>
             </div>
            ...
            <input name="price" className="border-slate-300 border focus:outline-none bg-transparent px-4 py-2 rounded-lg text-[16px] w-full" type="number" placeholder='টাকার পরিমাণ---' />
            </div>
            </div>
            <div>
            ...
            <button>{loading ? <Load />:"আপডেট করুন"}</button>
            </div>
          </div>
        </div>         
    </form>
