...
import { getWithPackageOrdersData } from "../../../../../../Api/customOrderRelatedApi/customApi";
import { useState } from "react";
import WithPackageCard from "./WithPackageCard";
const WithPackageOrders = () => {
    const [moreText,setMoreText] = useState(false);

    const {data:withPackageOrdersData,isLoading,refetch} = useQuery({
       ...
       <div className="md:p-5 p-3">
            ...
            <div className="overflow-x-auto">
                <table className="min-w-full bg-white border border-gray-300 shadow-md">
                    ...
                    <tbody>
                        {withPackageOrdersData?.map((order) => (
                            <WithPackageCard key={order._id} order={order} loading={isLoading} reFetch={refetch}/>
                        ))}
                    </tbody>
                </table>
            </div>
