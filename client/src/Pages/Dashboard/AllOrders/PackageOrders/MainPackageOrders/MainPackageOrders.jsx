...
import { getMainPackageAllOrders } from "../../../../../Api/PackageRelatedApi/packageApi";
...
const MainPackageOrders = () => {
  const [moreText,setMoreText] = useState(false);
    const {data:mainPackageOrderData,isLoading,refetch} = useQuery({
        queryKey:["mainPackageOrderData"],
        queryFn:async()=> await getMainPackageAllOrders()
    })
      console.log(mainPackageOrderData);
    ...
    return <>
     <div className="my-10">
       <div className=" mb-5">
           ...
           <p className="text-lg mb-2 text-primary font-medium">এখানে মেইন প্যাকেজের সকল অর্ডার এর লিস্ট</p>
                ...
                <table className="min-w-full bg-white border border-gray-300 shadow-md">
                    <thead>
                        <tr className="bg-gray-200 text-gray-700">
                            ...
                            <th className="text-sm md:text-base md:py-2 py-[4px] md:px-4 px-[6px] border">প্যাকেজের নাম</th>
                            <th className="text-sm md:text-base md:py-2 py-[4px] md:px-4 px-[6px] border">প্যাকেজে সংখ্যা</th>
                            ...
                            <th className="text-sm md:text-base md:py-2 py-[4px] md:px-4 px-[6px] border">প্যাকেজের সংখ্যা</th>
                    ...
                    <tbody>
                        {mainPackageOrderData?.map((order) => (
                            <MainPackageCard key={order._id} order={order} loading={isLoading} reFetch={refetch}/>
                        ))}
                    </tbody>
                </table>
            </div>
